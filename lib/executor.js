...
//
// Additionally, there is one special event:
// 8. Finally - Executes last, regardless of the event's outcome.
export default class Executor {
  ...
  async execute(event) {
    ...
    if (!event.canceled) {
      ...
      await this.eventHandlers[type](this.instance, event)
      await this.emit(`end-${type}`, event)
    }
    await this.emit(`finally-${type}`, event)
  }
}
