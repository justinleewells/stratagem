...
/**
 * A Unit property that may have a range of values.
 * @class
 */
class Resource {
  ...
  /**
   * Internally calculates maximum based on the resModType 
   * configuration.
  ...
  _calcMax() {
    ...
    if (config.resModType === 'integer') {
      this._max = this._base + this._mod;
    }
    else if (config.resModType === 'percent') {
