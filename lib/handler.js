...
class Handler {
  constructor({name, transient, properties, selector, initializers, effects, conditions, priority, envelop}) {
    this.name = name
  ...
  static initialize() {
    Executor.define(Config.get('triggerHandlerEventId'), ({instance, event}) => {
    ...
    if (results.length > 0) {
      results.forEach((properties) => {
        if (properties.transient) {
          properties.events.forEach(event => context.instance.execute(event))
        } else {
          context.instance.execute(new Event({
            properties,
            type: Config.get('triggerHandlerEventId')
          }))
        }
      })
    }
